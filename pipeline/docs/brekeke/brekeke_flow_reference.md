...
- 次は `コンテキスト設定`（saveContextModel2DB）へ

### 3.2 saveContextModel2DB（コンテキストモデル設定）— waitの直後に必ず配置
...
### 3.3b acceptance_times（受付時間チェック）— 任意。コンテキスト設定の直後に配置
...
遷移:
- `^true$` → acceptable → `冒頭アナウンス`（TTS）
- `^false$` → rejected → `時間外フラグ`
- `^TIMEOUT$`, `^ERROR$` → `時間外フラグ`

> 営業時間制御が不要なフローでは省略する。その場合 saveContextModel2DB の次が直接 冒頭アナウンス になる。
...
- **数字入力が可能な入力ステップでは必ずこちらを使う**
- 対象: 用件（番号選択）、生年月日、電話番号、診察券番号、その他数値を含む入力全般
- パラメータはAmiVoice Speech to Textと同様
...
- `functionCall` は必要に応じて設定する（contextDisplayType に関わらず任意）
- `contextDisplayType` が `"TEXT"` の場合は通常 `functionCall` を使わないため空文字のまま
...
|---|---|---|
| 標準（大半のステップ） | `{tts_g:恐れ入りますがご回答が確認できませんでした。今一度、}` | `{tts_g:かしこまりました。折り返しの際にお伺いいたします。}` |
| 用件確認（致命的） | 同上 | `{tts_g:ご回答の確認ができませんでしたのでこちらからお電話失礼させていただきます。}` |
| 氏名 | `{tts_g:うまく聞き取れませんでした。ゆっくり過ぎるとうまく聞き取れませんので、普通のスピードで、再度、フルネームをお話しください。}` | 標準 |
| 電話番号 | `{tts_g:恐れ入りますがご回答が確認できませんでした。はい か いいえで再度お答えいただけますでしょうか。}` | `{tts_g:ご回答の確認ができませんでしたので、現在通知いただいている電話番号へご連絡いたします。}` |
| その他お問い合わせ | `{tts_g:恐れ入りますがご回答が確認できませんでした。今一度おっしゃっていただけますでしょうか。}` | `{tts_g:かしこまりました。折り返しの際にお伺いいたします。}` |
| 空（無言終話） | `""` | `""` |
...
遷移ラベル: `非通知`, `固定`, `海外`, `携帯`, `WebRTC`, `その他`

> **2026-04 仕様更新**: `WebRTC` 分岐を追加。現状は固定・携帯と同じく通常フローに合流させる。将来 Pattern 5（WebRTC 専用シナリオ、個人情報サブフロー省略）を実装する時に別扱いにする想定。
...
| STT | `入力-{ステップ名}` | `入力-用件`, `入力-氏名` |
| OpenAI | `OpenAI-{ステップ名}` | `OpenAI-用件`, `OpenAI-氏名` |
| Retry | `リトライ-{ステップ名}` | `リトライ-用件` |
| save | `save-{項目名}` | `save-用件` |

> 実フローでは `-`（ハイフン）区切りが多用されている。新規作成時は `_` でも `-` でもよいが、1ステップ内で統一する。
...
```
冒頭(wait) → コンテキスト設定(saveContextModel2DB) → [営業時間チェック(acceptance_times)]
  ├── true（または acceptance_times なし）
  │     → 冒頭アナウンス(TTS) → 仕訳-電話番号(incoming-classifier)
  │           ├── 非通知 → 電話番号※非通知番号からの入電(TTS) → 非通知切断フラグ → 切断-非通知
  │           ├── 海外   → 海外からの入電(TTS) → 代表案内フラグ-海外 → 切断-国際電話
  │           └── 固定/携帯/WebRTC/その他 → 用件(TTS) → [用件確認フロー]
...
- [ ] `incoming-classifier` の分岐が非通知・海外・固定・携帯・WebRTC を網羅（6 分岐 + catch-all「その他」）
