...
END_キャンセル完了.prompt={tts_g:TODO_要確認（B-6）}
END_確認完了.prompt={tts_g:TODO_要確認（B-6）}
END_内容聴取完了.prompt={tts_g:TODO_要確認（B-6）}
END_当日キャンセル.prompt={tts_g:TODO_要確認（B-7: 代表番号 044-511-2112 を 3 回反復読み上げる形式。前後文言は customer_doc 確認後確定）}
冒頭_アナウンス.prompt={tts_g:TODO_要確認（B-4: customer_doc 未納入）}
用件確認.prompt={tts_g:TODO_要確認（B-5）}
現在の予約日.prompt={tts_g:TODO_要確認（B-5）}
内容詳細.prompt={tts_g:TODO_要確認（B-5）}
