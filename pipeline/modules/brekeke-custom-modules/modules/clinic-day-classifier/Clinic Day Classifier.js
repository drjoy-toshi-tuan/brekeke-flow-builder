...
// 去日受付 (acceptPastDay): 過去日を受け付けるか (yes / no)
//   no  → 過去日は PAST_DAY を返す
//   yes → 過去日も business として扱う (休診日判定は継続)
var acceptPastDay    = String($runner.getProperty("acceptPastDay") || "no").trim().toLowerCase() === "yes";
...
// 1日の分類: PAST / CLOSED(reason) / BLOCKED / BUSINESS
//   過去日 + acceptPastDay=no  → PAST
//   過去日 + acceptPastDay=yes → 休診日判定のみ実施 (リードタイムは対象外)
function classifyDate(isoDate) {
    if (isPast(isoDate)) {
        if (!acceptPastDay) return { type: "PAST" };
...
// 過去日判定 (yyyy-MM-dd の文字列比較で可)
function isPast(iso) { return iso < TODAY_ISO; }

// DTMF 4桁 MMdd の年補完
//   acceptPastDay=no  → 直近の未来日 (今年、過去なら翌年)
//   acceptPastDay=yes → 今年 / 翌年 のうち今日に近い方 (過去も可)
function resolveMMdd(mo, day) {
    var c0 = toIso(CURRENT_YEAR,     mo, day);
    var c1 = toIso(CURRENT_YEAR + 1, mo, day);
    if (!c0 && !c1) return null;

    if (!acceptPastDay) {
        if (c0 && c0 >= TODAY_ISO) return c0; // 今年でまだ過去でない
        if (c1) return c1;                     // 今年は過去 → 翌年
        return c0;
    }
    // yes: 今日に近い方
    var d0 = c0 ? dayDiff(c0, TODAY_ISO) : Infinity;
    var d1 = c1 ? dayDiff(c1, TODAY_ISO) : Infinity;
    if (d0 === Infinity && d1 === Infinity) return null;
    return (d0 <= d1) ? c0 : c1;
}

// 具体的な日付を抽出 (yyyy-MM-dd の配列。最大3件、重複除去済み)
function extractConcreteDates(text) {
    ...
    if (dtmf8) {
        ...
        return [iso8]; // 過去日かどうかは後続の classifyDate で判定 (PAST_DAY)
    }

    // DTMF: 4桁数字 MMdd (年は acceptPastDay に基づき補完)
    ...
    // パターン0: yyyy-MM-dd hh:mm
    var p0 = /(\d{4})-(\d{2})-(\d{2}) \d{2}:\d{2}/g;
    while ((m = p0.exec(text)) !== null) results.push(m[1] + "-" + m[2] + "-" + m[3]);
    if (results.length > 0) return dedupe(results);

    // パターン3: yyyy/M/d
    var p3 = /(\d{4})\/(\d{1,2})\/(\d{1,2})/g;
    while ((m = p3.exec(text)) !== null) results.push(toIso(parseInt(m[1]), parseInt(m[2]), parseInt(m[3])));
    if (dedupe(results).length > 0) return dedupe(results);

    // パターン1: yyyy年M月d日
    var p1 = /(\d{4})年(\d{1,2})月(\d{1,2})日/g;
    while ((m = p1.exec(text)) !== null) results.push(toIso(parseInt(m[1]), parseInt(m[2]), parseInt(m[3])));

    // パターン2: M月d日 (年なし) — 年は acceptPastDay に基づき補完 (resolveMMdd と共通)
    //   no  → 直近の未来日 (今日以降はそのまま今年、今日より過去なら翌年)
    //   yes → 今年 / 翌年 のうち今日に近い方 (過去日も可)
    var textNoYear = text.replace(/\d{4}年\d{1,2}月\d{1,2}日/g, "");
    var p2 = /(\d{1,2})月(\d{1,2})日/g;
    while ((m = p2.exec(textNoYear)) !== null) {
        var p2iso = resolveMMdd(parseInt(m[1], 10), parseInt(m[2], 10));
        if (p2iso) results.push(p2iso);
    }

    // パターン4: 来月・再来月 + 日付 (複数対応)
    // 「再来月」を先に判定できる単一正規表現で double-match バグを回避
    var mpRe = /(再来月|来月)(?:の)?(\d{1,2})日((?:[とや、・]\d{1,2}日)*)/g;
    var mpMatch;
    while ((mpMatch = mpRe.exec(text)) !== null) {
        var offset   = (mpMatch[1] === "再来月") ? 2 : 1;
    ...
    // 週指定なし: 単体曜日 (X曜日に / 月曜日など)
    if (!dowFound) {
        var pDowSingle = /([月火水木金土日]曜日)/g;
        var ds;
...
function dedupe(arr) {
    ...
    if (unique.length > 3) {
        $runner.getLogger().warn("[checkClosedDay] Truncated to 3 dates from " + unique.length);
...
// 曖昧な月指定 (具体的な日なし) の判定
//   "vague": 月コンテキストあり (X月 / 今月 / 来月 / 再来月, 旬あり/なし問わず)
//   "none" : 月コンテキストなし (旬のみで月がない場合を含む)
...
// available_date は常に計算 (どのブランチでも案内に使えるよう先に設定)
...
function buildBusinessOutput(concreteFirstIso) {
    if (outputType === "フリーテキスト") {
        return rawInputTrimmed || "NO_RESULT";
    }
...
if (UNKNOWN_DATE_RE.test(rawInput) || NO_PREFERENCE_RE.test(rawInput) || BARE_NEGATION_RE.test(_bareNeg)) {
    $runner.getLogger().info("[checkClosedDay] Unknown / no-preference date input detected — returning 不明");
    $runner.setResult("不明");

} else {
    // Step1: 具体的な日付をローカルパース
    ...
    if (parsedDates.length > 0) {
        ...
        if (hasPast) {
            // acceptPastDay=no かつ 過去日を含む → PAST_DAY
            $runner.getLogger().info("[checkClosedDay] Result: PAST_DAY — " + closedDates.join(", "));
            $runner.setResult("PAST_DAY");
        } else if (closedDates.length === parsedDates.length) {
            // 全件が休診/受付不可 のときのみ NON_BUSINESS_DAY
            $runner.getLogger().info("[checkClosedDay] Result: NON_BUSINESS_DAY — " + closedDates.join(", "));
            $runner.setResult("NON_BUSINESS_DAY");
        } else {
            var out = buildBusinessOutput(parsedDates[0]);
            $runner.getLogger().info("[checkClosedDay] Result: BUSINESS (output_type=" + outputType + ") — " + out);
