# -*- coding: utf-8 -*-
"""checkup_menu_classifier 受入テスト — acceptance_test/cases.tsv（テストが正）を全件照合。
...
def main():
    ...
    fails = []
    for c in cases:
        got = classify(c["utterance"], c["menu"])
        if got != c["expected"]:
            fails.append((c, got))
    print("checkup_menu_classifier oracle acceptance: %d/%d PASS"
          % (len(cases) - len(fails), len(cases)))
    if fails:
        print("\nFAIL %d 件:" % len(fails))
        for c, got in fails:
            print("  %s (L%d) [%s]: %r -> got=%s expected=%s [%s]"
                  % (c["id"], c["_lineno"], c["menu"], c["utterance"], got,
