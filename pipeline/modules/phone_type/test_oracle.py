# -*- coding: utf-8 -*-
"""phone_type 受入テスト — acceptance_test/cases.tsv（テストが正）を全件照合。
...
def main():
    ...
    fails = []
    for c in cases:
        got = classify(c.get("utterance", ""))
        if got != c["expected"]:
            fails.append((c, got))
    print("phone_type oracle acceptance: %d/%d PASS"
          % (len(cases) - len(fails), len(cases)))
    if fails:
        print("\nFAIL %d 件:" % len(fails))
        for c, got in fails:
            print("  %s (L%d): %r -> got=%s expected=%s [%s]"
                  % (c["id"], c["_lineno"], c.get("utterance", ""), got,
