// n_choice — 残薬確認(あり/なし/処方されていない) config 充填版（A3 Phase2 調達）
// 正本 modules/n_choice/script.js（engine v4）に「残薬確認」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= 残薬確認データ（3択・compound あり）。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "residualMedication";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var DTMF_MAP = {"1":"あり","2":"なし","3":"処方されていない"};
    // @spec-end
    ...
    // @spec-begin
    var COMPOUND_PATTERNS = [{"regex":"処方されてい|処方されてな|薬は出てい|薬が出てい|処方なし|処方無","result":"処方されていない"}];
    // @spec-end
    if (result === null) {
    ...
    // @spec-begin
    var KEYWORD_PATTERNS = [{"regex":"ありません|残っていません|残ってない|飲み切|なし|ない","result":"なし"},{"regex":"あり|残って|余って|まだある","result":"あり"}];
    // @spec-end
    if (result === null) {
