// n_choice — 当日確認(本日/別日) config 充填版（A3 Phase2 調達）
// 正本 modules/n_choice/script.js（engine v4）に「当日確認_別日本日」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= 当日確認データ。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "todayReservation";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var KEYWORD_PATTERNS = [{"regex":"別日|別の日|明日以降|あした|明日|来週|他の日|後日|翌日","result":"別日"},{"regex":"本日|今日|きょう|当日","result":"本日"}];
    // @spec-end
    if (result === null) {
