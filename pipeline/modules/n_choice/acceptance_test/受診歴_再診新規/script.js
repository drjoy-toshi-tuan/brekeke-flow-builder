// n_choice — 受診歴(再診/新規) config 充填版（A3 Phase2 調達）
// 正本 modules/n_choice/script.js（engine v4）に「受診歴_再診新規」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= 受診歴データ。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "visitHistory";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var KEYWORD_PATTERNS = [{"regex":"再診|さいしん|前にかかった|通院|かかりつけ|以前","result":"再診"},{"regex":"新規|初めて|はじめて|初診|今回が初","result":"新規"}];
    // @spec-end
    if (result === null) {
