// n_choice — 薬処方確認(飲んでいる/飲んでいない) config 充填版（acceptance: 服薬有無 spec）
// 正本 modules/n_choice/script.js（engine v4）に「薬処方確認_変更キャンセル」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= カレス記念病院_診療 薬処方確認データ。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "medicationTaking";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var TOKEN_MAP = [{"regex": "^飲んでいる$", "result": "飲んでいる"}, {"regex": "^飲んでいない$", "result": "飲んでいない"}];
    // @spec-end
    if (result === null) {
    ...
    // @spec-begin
    var COMPOUND_PATTERNS = [{"regex": "(飲んでいる|飲んでます|のんでいる|はい)", "result": "飲んでいる"}, {"regex": "(飲んでいない|飲んでません|のんでいない|いいえ)", "result": "飲んでいない"}];
    // @spec-end
    if (result === null) {
