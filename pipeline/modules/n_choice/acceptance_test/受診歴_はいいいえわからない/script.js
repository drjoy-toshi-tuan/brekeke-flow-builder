// n_choice — 受診歴確認(はい/いいえ/わからない) config 充填版（acceptance: 受診歴_はいいいえわからない spec）
// 正本 modules/n_choice/script.js（engine v4）に「受診歴確認」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= カレス記念病院_診療 受診歴確認データ。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// いいえ を先に評価する（はい の「はじめて/初めて」が「はじめてではない/初めてではない」の
// 部分文字列に一致するシャドーを避けるため）。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "firstVisit";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var DTMF_MAP = {"2": "いいえ", "1": "はい", "3": "わからない"};
    // @spec-end
    ...
    // @spec-begin
    var TOKEN_MAP = [{"regex": "^いいえ$", "result": "いいえ"}, {"regex": "^はい$", "result": "はい"}, {"regex": "^わからない$", "result": "わからない"}];
    // @spec-end
    if (result === null) {
    ...
    // @spec-begin
    var COMPOUND_PATTERNS = [{"regex": "(いいえ|はじめてではない|初めてではない|二回目|2回目|再診|前にも|通っている|通っています)", "result": "いいえ"}, {"regex": "(はい|はじめて|初めて|初診|初めてです)", "result": "はい"}, {"regex": "(わからない|わかりません|分からない|分かりません|不明)", "result": "わからない"}];
    // @spec-end
    if (result === null) {
