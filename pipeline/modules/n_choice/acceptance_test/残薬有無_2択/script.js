// n_choice — 残薬確認(はい/いいえ・2択版) config 充填版（acceptance: 残薬有無_2択 spec）
// 正本 modules/n_choice/script.js（engine v4）に「残薬確認(2択)」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= カレス記念病院_診療 残薬確認データ。既存 残薬確認 spec（3択・
// あり/なし/処方されていない・ZANYAKU）とは別 spec。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "remainingDrugs";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var TOKEN_MAP = [{"regex": "^はい$", "result": "はい"}, {"regex": "^いいえ$", "result": "いいえ"}];
    // @spec-end
    if (result === null) {
    ...
    // @spec-begin
    var COMPOUND_PATTERNS = [{"regex": "(はい|あります|ある)", "result": "はい"}, {"regex": "(いいえ|ありません|ない)", "result": "いいえ"}];
    // @spec-end
    if (result === null) {
