// n_choice — 顧客種別(企業/個人) config 充填版（A3 Phase2 調達）
// 正本 modules/n_choice/script.js（engine v4）に「顧客種別_企業個人」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= 顧客種別データ。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "customerType";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var KEYWORD_PATTERNS = [{"regex":"企業|法人|会社|勤め先|勤務先","result":"企業"},{"regex":"個人|自費|プライベート","result":"個人"}];
    // @spec-end
    if (result === null) {
