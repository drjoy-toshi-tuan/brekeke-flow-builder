// n_choice — 予約手段(ネット/電話) config 充填版（A3 Phase2 調達）
// 正本 modules/n_choice/script.js（engine v4）に「予約手段_ネット電話」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= 予約手段データ。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "reservationMethod";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var KEYWORD_PATTERNS = [{"regex":"ネット|インターネット|ウェブ|web|オンライン","result":"ネット"},{"regex":"電話","result":"電話"}];
    // @spec-end
    if (result === null) {
