// n_choice — 変更内容(受診内容/日程変更) config 充填版（A3 Phase2 調達）
// 正本 modules/n_choice/script.js（engine v4）に「変更内容_受診日程」spec を充填した具体版。
// engine は正本と同一（engine_hash 一致で部品種別=n_choice と認定）。
// spec（@spec ブロック）= 変更内容データ。{{INPUT_MODULE}} は P6 matrix が case ごとに充填。
// @part-id: n_choice
// @engine-version: v4

var logger = $runner.getLogger();
var INPUT_MODULE = "{{INPUT_MODULE}}";
var CONTEXT_NAME = "changeContent";
...
if (input === null || input === undefined || input === "") {
    ...
    // @spec-begin
    var KEYWORD_PATTERNS = [{"regex":"日程|日にち|日時|予約日|スケジュール|曜日","result":"日程変更"},{"regex":"受診内容|内容|コース|検査内容","result":"受診内容"}];
    // @spec-end
    if (result === null) {
