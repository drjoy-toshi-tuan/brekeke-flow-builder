// BusinessHour Classifier — @General$Script モジュールに直接貼り付けて使う版
// 着信日時 (timeStart もしくは指定値) と施設の営業ルールを照合し、
//   営業中 / 営業時間外 / 定休日 / 祝日 / 固定休 / ERROR の 6 分岐に仕分ける。
// 設計責務: 4 層責任モデル ([[project_4layer_responsibility_model]]) における分岐層。
// jumps 設定 (Brekeke モジュール UI 側で設定):
//   ^営業中$ / ^営業時間外$ / ^定休日$ / ^祝日$ / ^固定休$ / ^ERROR$
// @part-id: business_hour_classifier
// @engine-version: v1
...
// @spec-begin
var WEEKDAY_SCHEDULE = "mon=09:00-18:00,tue=09:00-18:00,wed=09:00-18:00,thu=09:00-18:00,fri=09:00-18:00,sat=closed,sun=closed";
// @spec-end

// 毎年の固定休 (mm-dd 列): 年末年始、お盆休み、創立記念日等。空文字なら無し
// @spec-begin
var CLOSED_DATES = "12-29,12-30,12-31,01-02,01-03";
// @spec-end

// 国民の祝日扱い: "closed"=祝日休 / "open"=祝日も営業 (ER対応病院等)
// @spec-begin
var NATIONAL_HOLIDAY = "closed";
// @spec-end

// 祝日マスタ Note 名 (Brekeke 管理画面 Note。多年同居、年初末尾追記運用)
// @spec-begin
var HOLIDAY_NOTE_NAME = "drjoy.holidays";
// @spec-end
...
// 過去日判定の基準「今日」: TARGET_DATETIME と同じ書式 ("now" / リテラル日時 / "<%var%>")
//   "now"                       ← 着信時刻 (timeStart) を基準にする ★既定、本番は必ずこれ
//   "yyyy-MM-dd HH:mm:ss" 等    ← 基準日を固定 (Pattern 6 決定論テストで「今日」を注入する用)
//   "<%contextName%>"           ← session context から基準日を読む
// 予約受付の特性上「過去その日時に営業していたか」を返す必要はないため、
// TARGET_DATETIME の日付がこの基準日より前 (= 過去日) なら、曜日/祝日/固定休に
// 関わらず一律「営業時間外」に倒す。比較は日付単位 (時刻は無視) — 同じ「今日」内の
// 過ぎた時刻枠は過去扱いせず通常判定する (2026-06-04 確定)。
