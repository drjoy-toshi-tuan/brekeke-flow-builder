...
// ── Icon cho tính năng AI Chat (trợ lý sửa flow) ──────────────────────────────
// Bổ sung các icon chưa có trong subset trim sẵn (addCollection cùng prefix GỘP vào
// bộ đã đăng ký). Body lấy nguyên bản từ @iconify-json/* trên npm.

// fluent:arrow-enter-left-24-filled — nút gửi tin nhắn (↵).
addCollection({
  prefix: 'fluent',
  width: 24,
  height: 24,
  icons: {
    'arrow-enter-left-24-filled': {
...
// mingcute:square-line — nút DỪNG khi AI đang trả lời (ai-fill đã có sẵn trong subset).
addCollection({
  prefix: 'mingcute',
  width: 24,
  height: 24,
...
// svg-spinners:3-dots-bounce — hiệu ứng "đang suy nghĩ" của AI (subset đã có prefix).
addCollection({
  prefix: 'svg-spinners',
  width: 24,
  height: 24,
  icons: {
    '3-dots-bounce': {
      body: '<circle cx="4" cy="12" r="3" fill="currentColor"><animate id="bkDot0" attributeName="cy" begin="0;bkDot1.end+0.25s" calcMode="spline" dur="0.6s" keySplines=".33,.66,.66,1;.33,0,.66,.33" values="12;6;12"/></circle><circle cx="12" cy="12" r="3" fill="currentColor"><animate attributeName="cy" begin="bkDot0.begin+0.1s" calcMode="spline" dur="0.6s" keySplines=".33,.66,.66,1;.33,0,.66,.33" values="12;6;12"/></circle><circle cx="20" cy="12" r="3" fill="currentColor"><animate id="bkDot1" attributeName="cy" begin="bkDot0.begin+0.2s" calcMode="spline" dur="0.6s" keySplines=".33,.66,.66,1;.33,0,.66,.33" values="12;6;12"/></circle>',
    },
  },
});

// proicons:openai — logo OpenAI (avatar AI trong chat, cũng là icon type node OpenAI).
addCollection({
  prefix: 'proicons',
  width: 24,
  height: 24,
...
// heroicons-solid:light-bulb — icon "ý tưởng" ở header thẻ đề xuất thay đổi.
addCollection({
...
// lets-icons:edit-fill — icon thao tác "sửa node" trong danh sách đề xuất.
addCollection({
  prefix: 'lets-icons',
  width: 24,
  height: 24,
...
// qlementine-icons:plus-16 — icon thao tác "thêm" (node/dây) trong danh sách đề xuất.
addCollection({
